...
describe('YamswapFactory', () => {
  ...
  it('createPair:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(0x265c07)
  })
  ...
  it('setFeeToSetter', async () => {
    await expect(factory.connect(other).setFeeToSetter(other.address)).to.be.revertedWith('Yamswap: FORBIDDEN')
    await factory.setFeeToSetter(other.address)
    expect(await factory.feeToSetter()).to.eq(other.address)
    await expect(factory.setFeeToSetter(wallet.address)).to.be.revertedWith('Yamswap: FORBIDDEN')
  })
})